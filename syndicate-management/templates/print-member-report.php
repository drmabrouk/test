...
<head>
    <meta charset="UTF-8">
    <title>تقرير العضو الانضباطي - <?php echo esc_html($member->name); ?></title>
    ...
    <style>
        ...
        .member-info { display: grid; grid-template-columns: auto 1fr auto; gap: 30px; margin-bottom: 40px; border: 1px solid #E2E8F0; padding: 25px; border-radius: 12px; align-items: center; background: #F8FAFC; }
    ...
    <?php else: ?>
        <div class="report-header" style="border-bottom: 3px double #111F35; padding-bottom: 20px;">
            <div style="text-align: right; flex: 1;">
                ...
                <h2 style="margin: 8px 0; color: #4A5568; font-weight: 800; font-size: 18px; border-top: 1px solid #eee; padding-top: 5px;"><?php echo esc_html($syndicate['syndicate_name']); ?></h2>
            </div>
            <div style="flex: 1; text-align: center;">
                <?php if (!empty($syndicate['syndicate_logo'])): ?>
                    <img src="<?php echo esc_url($syndicate['syndicate_logo']); ?>" style="max-height: 100px; width: auto; object-fit: contain; display: block; margin: 0 auto;">
                <?php endif; ?>
            </div>
            <div style="text-align: left; flex: 1;">
                ...
                <p style="margin: 5px 0; font-size: 10px; color: #718096;">الرقم المرجعي: <?php echo 'REP-' . date('Ym') . '-' . $member->id; ?></p>
            </div>
        </div>
    <?php endif; ?>
    ...
    <div class="member-info">
        <div style="text-align: center;">
            <?php if ($member->photo_url): ?>
                <img src="<?php echo esc_url($member->photo_url); ?>" style="width: 110px; height: 110px; border-radius: 12px; object-fit: cover; border: 3px solid #fff; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            <?php else: ?>
    ...
    <table>
        ...
        <tbody>
            <?php if (empty($records)): ?>
                <tr><td colspan="5">لا توجد سجلات مسجلة لهذا العضو.</td></tr>
            <?php else: ?>
