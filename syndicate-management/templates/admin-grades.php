...
<div class="sm-grades-management" dir="rtl">
        ...
        <div style="background: #fff; padding: 25px; border-radius: 12px; border: 1px solid var(--sm-border-color); margin-bottom: 30px;">
            <div style="display: grid; grid-template-columns: 2fr 1fr 1fr 1fr auto; gap: 15px; align-items: end;">
                <div class="sm-form-group" style="margin-bottom:0;">
                    ...
                    <select id="grade-member-id" class="sm-select">
                ...
                <button onclick="saveMemberGrade()" class="sm-btn" style="height: 45px; background: var(--sm-primary-color);">رصد الدرجة</button>
            </div>
        </div>

        <div id="grades-table-container">
            <div style="padding: 40px; text-align: center; background: #f8fafc; border-radius: 12px; color: var(--sm-text-gray);">يرجى اختيار عضو لعرض درجاته.</div>
        </div>
    </div>
    ...
    <div id="class-grading" class="sm-internal-tab" style="display:none;">
        <div style="background: #fff; padding: 25px; border-radius: 12px; border: 1px solid var(--sm-border-color); margin-bottom: 30px;">
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr auto; gap: 15px; align-items: end;">
                <div class="sm-form-group" style="margin-bottom:0;">
                    ...
                    <select id="batch-class" class="sm-select" onchange="loadBatchMembers()">
                        <option value="">-- اختر الصف --</option>
                        <?php
                        global $wpdb;
                        $classes = $wpdb->get_results("SELECT DISTINCT class_name FROM {$wpdb->prefix}sm_members ORDER BY class_name ASC");
        ...
        <div id="batch-members-container"></div>
    </div>
...
<script>
document.getElementById('grade-member-id').addEventListener('change', function() {
    loadMemberGrades(this.value);
});

function loadMemberGrades(memberId) {
    if (!memberId) {
        document.getElementById('grades-table-container').innerHTML = '<div style="padding: 40px; text-align: center; background: #f8fafc; border-radius: 12px; color: var(--sm-text-gray);">يرجى اختيار عضو لعرض درجاته.</div>';
        return;
    }
...
function loadBatchMembers() {
    ...
    const container = document.getElementById('batch-members-container');
    container.innerHTML = 'جاري التحميل...';

    // Reuse search logic to get class members
    fetch('<?php echo admin_url('admin-ajax.php'); ?>?action=sm_search_members&query=' + encodeURIComponent(className))
    .then(r => r.json())
    .then(res => {
        if (res.success) {
            let html = '<table class="sm-table"><thead><tr><th>العضو</th><th>الدرجة</th></tr></thead><tbody>';
            res.data.forEach(s => {
                html += `<tr><td>${s.name}</td><td><input type="text" class="sm-input batch-grade-input" data-member-id="${s.id}" style="width:100px;"></td></tr>`;
            });
            html += '</tbody></table>';
...
function saveBatchGrades() {
    ...
    document.querySelectorAll('.batch-grade-input').forEach(input => {
        grades[input.dataset.memberId] = input.value;
    });
...
function renderGradesTable(grades) {
    ...
    if (grades.length === 0) {
        container.innerHTML = '<div style="padding: 40px; text-align: center; background: #f8fafc; border-radius: 12px; color: var(--sm-text-gray);">لا يوجد درجات مسجلة لهذا العضو.</div>';
        return;
    }
...
function deleteGrade(gradeId) {
    ...
    .then(res => {
        if (res.success) {
            ...
            loadMemberGrades(document.getElementById('grade-member-id').value);
        }
    });
}
</script>
