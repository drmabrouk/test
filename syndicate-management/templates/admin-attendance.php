...
<div class="sm-attendance-page" dir="rtl">
    ...
    <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 40px;">
        ...
        <div class="sm-stat-card" style="padding: 15px; background: #f8fafc; border: 1px solid #e2e8f0;">
            <div style="font-size: 11px; color: var(--sm-text-gray); font-weight: 700;">إجمالي الأعضاء</div>
            <div style="font-size: 1.8em; font-weight: 900; color: var(--sm-dark-color);"><?php echo $total_members; ?></div>
        </div>
            ...
            <div style="flex: 2; display: flex; align-items: center; gap: 25px; cursor: pointer;" onclick="openAttendanceModal('<?php echo esc_js($card['class_name']); ?>', '<?php echo esc_js($card['section']); ?>')">
                ...
                <div>
                    ...
                    <div style="font-size: 12px; color: var(--sm-text-gray); font-weight: 700; margin-top: 4px;">
                        إجمالي الأعضاء: <?php echo $card['member_count']; ?>
                    </div>
                </div>
            </div>
...
<div id="sm-attendance-marking-modal" class="sm-modal-overlay">
    <div class="sm-modal-content" style="max-width: 700px;">
        ...
        <div id="attendance-members-list" style="max-height: 400px; overflow-y: auto;">
            <!-- Loaded via AJAX -->
            <div style="text-align: center; padding: 40px; color: var(--sm-text-gray);">جاري تحميل قائمة الأعضاء...</div>
        </div>
...
function openAttendanceModal(className, section) {
    ...
    loadAttendanceMembers(className, section, date);
}
...
function loadAttendanceMembers(className, section, date) {
    const listContainer = document.getElementById('attendance-members-list');
...
function renderMembersList(members) {
    const listContainer = document.getElementById('attendance-members-list');
    if (members.length === 0) {
        listContainer.innerHTML = '<div style="padding: 20px; text-align: center;">لا يوجد أعضاء في هذا الصف.</div>';
        return;
    }
    ...
    members.forEach(s => {
        ...
        html += `
            <tr data-member-id="${s.id}">
                <td style="width: 50px;">${photo}</td>
                <td>
                    <div style="font-weight: 700; font-size: 13px;">${s.name}</div>
                    <div style="font-size: 11px; color: var(--sm-text-gray);">${s.member_code}</div>
                </td>
...
function saveAttendance(memberId, status, btn) {
    const date = document.getElementById('attendance-filter-date').value;
