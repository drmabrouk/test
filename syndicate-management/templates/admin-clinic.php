...
$pending_referrals = $wpdb->get_results("
    SELECT c.*, s.name as member_name, s.class_name, s.section, u.display_name as referrer_name
...
<div class="sm-clinic-module" dir="rtl">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        <h3 style="margin: 0; font-weight: 800;">العيادة النقابية</h3>
    ...
    <div style="margin-bottom: 40px;">
        <h4 style="border-bottom: 2px solid var(--sm-primary-color); padding-bottom: 10px; margin-bottom: 20px;">الأعضاء المحولون (بانتظار الوصول)</h4>
        <?php if (empty($pending_referrals)): ?>
            <div style="padding: 40px; text-align: center; background: #f8fafc; border-radius: 12px; color: var(--sm-text-gray);">لا يوجد أعضاء محولون حالياً.</div>
        <?php else: ?>
                    ...
                    <tbody>
                        <?php foreach ($pending_referrals as $r): ?>
                            <tr>
                                ...
                                <td>
                                    <div style="font-weight: 800;"><?php echo esc_html($r->member_name); ?></div>
                ...
                <table class="sm-table">
                    ...
                    <tbody>
                        <?php foreach ($history as $h): ?>
                            <tr>
                                <td>
                                    <div style="font-weight: 800;"><?php echo esc_html($h->member_name); ?></div>
...
<div id="referral-modal" class="sm-modal-overlay">
    <div class="sm-modal-content" style="max-width: 500px;">
        <div class="sm-modal-header">
            <h3>تحويل عضو للعيادة</h3>
            <button class="sm-modal-close" onclick="document.getElementById('referral-modal').style.display='none'">&times;</button>
        </div>
        <div class="sm-form-group">
            <label class="sm-label">البحث عن العضو:</label>
            <input type="text" id="clinic-member-search" class="sm-input" placeholder="اكتب اسم العضو أو كوده..." onkeyup="clinicSearchMembers(this.value)">
        ...
        <div id="selected-member-box" style="display: none; background: #f0fdf4; padding: 15px; border-radius: 8px; border: 1px solid #bbf7d0; margin-bottom: 20px;">
            <div style="font-weight: 800;" id="selected-member-name"></div>
            <div style="font-size: 12px; color: #166534;" id="selected-member-info"></div>
...
function clinicSearchMembers(query) {
    if (query.length < 2) {
    ...
    .then(res => {
        if (res.success && res.data.length > 0) {
            let html = '';
            res.data.forEach(s => {
                html += `<div style="padding: 10px; border-bottom: 1px solid #eee; cursor: pointer;" onclick="selectClinicMember(${s.id}, '${s.name}', '${s.class_name} ${s.section}')">
                            <strong>${s.name}</strong> (${s.member_code})<br><small>${s.class_name} - ${s.section}</small>
                         </div>`;
            });
...
function selectClinicMember(id, name, info) {
    document.getElementById('selected-member-id').value = id;
    document.getElementById('selected-member-name').innerText = name;
    document.getElementById('selected-member-info').innerText = info;
    document.getElementById('selected-member-box').style.display = 'block';
    document.getElementById('clinic-search-results').style.display = 'none';
    document.getElementById('clinic-member-search').value = '';
}

function submitClinicReferral() {
    const id = document.getElementById('selected-member-id').value;
    if (!id) { alert('يرجى اختيار عضو أولاً'); return; }
...
function confirmClinicArrival(referralId) {
    ...
    .then(res => {
        if (res.success) {
            smShowNotification('تم تأكيد وصول العضو');
            setTimeout(() => location.reload(), 500);
        }
    });
}
...
document.addEventListener('click', function(e) {
    ...
    if (results && !results.contains(e.target) && e.target.id !== 'clinic-member-search') {
