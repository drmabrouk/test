...
<div class="sm-content-wrapper" dir="rtl">
        ...
        <table class="sm-table">
            ...
            <tbody>
                ...
                <?php else: ?>
                    ...
                    ?>
                        <tr>
                            <td style="font-weight: 700;"><?php echo esc_html($row->member_name); ?><br><small style="color:#718096;"><?php echo SM_Settings::format_grade_name($row->class_name, $row->section, 'short'); ?></small></td>
                            ...
                            <td>
                                <div style="display: flex; gap: 8px;">
                                    <?php if ($row->status == 'held'): ?>
                                        <button onclick="markReturned(<?php echo $row->id; ?>)" class="sm-btn" style="background:#38a169; padding:5px 10px; font-size:11px; width:auto;">إعادة للعضو</button>
                                    <?php endif; ?>
            ...
            <form id="add-confiscated-form">
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
                    <div class="sm-form-group" style="grid-column: span 2;">
                        <label class="sm-label">العضو المعني:</label>
                        <select name="member_id" class="sm-select" required>
                            <option value="">-- اختر العضو من القائمة --</option>
                            <?php 
                            $members = SM_DB::get_members();
                            foreach($members as $s) echo '<option value="'.$s->id.'">'.$s->name.' ('.SM_Settings::format_grade_name($s->class_name, $s->section, 'short').')</option>';
                            ?>
                        </select>
                    </div>
    ...
    <div id="return-confirmation-modal" class="sm-modal-overlay">
        <div class="sm-modal-content" style="max-width: 400px; text-align: center;">
            ...
            <p style="color: #4A5568; font-size: 14px; margin-bottom: 25px;">هل تم التأكد من تسليم المادة المصادرة للعضو أو ولي أمره بشكل رسمي؟</p>
