...
<div class="sm-content-wrapper" dir="rtl">
    ...
    <div class="sm-parents-rows-container" style="display: flex; flex-direction: column; gap: 15px;">
        ...
        if ($search) {
            ...
            // Advanced Search: Join with members and check meta
            global $wpdb;
            $extra_parent_ids = $wpdb->get_col($wpdb->prepare(
                "SELECT DISTINCT parent_user_id FROM {$wpdb->prefix}sm_members WHERE (name LIKE %s OR parent_email LIKE %s) AND parent_user_id IS NOT NULL",
        ...
        <?php else: ?>
            <?php foreach ($parents as $parent): 
                $children = SM_DB::get_members_by_parent($parent->ID);
            ?>
    ...
    <div id="add-parent-modal" class="sm-modal-overlay">
        <div class="sm-modal-content">
            ...
            <form id="add-parent-form">
                ...
                <p style="font-size:12px; color:#718096; margin-top:15px;">ملاحظة: لربط ولي الأمر بعضو، قم بتحرير بيانات العضو من قسم "إدارة الأعضاء".</p>
    ...
    <div id="call-in-modal" class="sm-modal-overlay">
        <div class="sm-modal-content" style="max-width: 500px;">
            ...
            <div style="text-align: center; padding: 20px 0;">
                <p style="font-size: 1.1em; margin-bottom: 25px;">إرسال طلب حضور للنقابة لولي الأمر:<br><strong id="call_in_parent_name" style="color: var(--sm-primary-color); font-size: 1.2em;"></strong></p>
    ...
    function sendCallViaEmail() {
        const msg = encodeURIComponent(document.getElementById('call_in_msg_text').value);
        const subject = encodeURIComponent('طلب استدعاء رسمي من النقابة');
