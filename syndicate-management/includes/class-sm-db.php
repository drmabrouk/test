...
class SM_DB {
    public static function get_members($filters = array()) {
        ...
        // Optimized: Reduced wildcard usage for member_code if it looks like a code
        $query = "SELECT * FROM {$wpdb->prefix}sm_members WHERE 1=1";
        ...
        if (!empty($filters['search'])) {
            ...
            if (preg_match('/^MB[0-9]+$/i', $search_str)) {
                $query .= $wpdb->prepare(" AND (member_code = %s OR $name_sql LIKE %s)", $search_str, $search_like);
            } else {
                $query .= $wpdb->prepare(" AND ($name_sql LIKE %s OR member_code LIKE %s OR class_name LIKE %s OR section LIKE %s)", $search_like, $search_like, $search_like, $search_like);
            }
        }
        ...
        if (!empty($filters['teacher_id']) && !empty($filters['include_reported'])) {
            ...
            $query .= $wpdb->prepare(" AND (teacher_id = %d OR id IN (SELECT DISTINCT member_id FROM {$wpdb->prefix}sm_records WHERE teacher_id = %d))", $tid, $tid);
            ...
            $query .= $wpdb->prepare(" AND id IN (SELECT DISTINCT member_id FROM {$wpdb->prefix}sm_records WHERE teacher_id = %d)", $teacher_id);
        }
    ...
    public static function generate_member_code() {
        global $wpdb;
        $last_code = $wpdb->get_var("SELECT member_code FROM {$wpdb->prefix}sm_members WHERE member_code LIKE 'MB%' ORDER BY member_code DESC LIMIT 1");
        ...
        return 'MB' . str_pad($next_number, 5, '0', STR_PAD_LEFT);
    }

    public static function add_member($name, $class, $email, $code = '', $parent_user_id = null, $teacher_id = null, $section = '', $extra = array()) {
        global $wpdb;
        ...
        // AUTO-GENERATE UNIFIED WP USER (Member)
        if (!$parent_user_id) {
            $username = $code;
            if (!username_exists($username)) {
                ...
                $email_addr = $code . '@syndicate-system.local'; // Automated email generation

                $user_id = wp_create_user($username, $password, $email_addr);
                if (!is_wp_error($user_id)) {
                    $wp_user = new WP_User($user_id);
                    $wp_user->set_role('sm_member');
                    $parent_user_id = $user_id;

                    update_user_meta($user_id, 'sm_temp_pass', $password);
                    wp_update_user(array('ID' => $user_id, 'display_name' => "Member $name"));
                }
            } else {
        ...
        SM_Logger::log('إضافة عضو', "الاسم: $name، الصف: $class، الشعبة: $section");
        $success = $wpdb->insert(
    ...
    public static function update_member($id, $data) {
        global $wpdb;
        SM_Logger::log('تعديل بيانات عضو', "معرف العضو: $id");
        return $wpdb->update(
            "{$wpdb->prefix}sm_members",
            array(
    ...
    public static function add_record($data, $skip_log = false) {
        ...
        if (!empty($violation_code)) {
            $count = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->prefix}sm_records WHERE member_id = %d AND violation_code = %s",
                $member_id, $violation_code
            ));
        ...
        if (!$skip_log) {
            SM_Logger::log('تسجيل مخالفة', "معرف العضو: $member_id، النوع: {$data['type']}، الدرجة: $degree");
        }
        ...
        if ($inserted && $status === 'accepted') {
            // Update member points and case file
            $wpdb->query($wpdb->prepare(
                "UPDATE {$wpdb->prefix}sm_members SET behavior_points = behavior_points + %d WHERE id = %d",
                $points, $member_id
            ));

            $total_points = $wpdb->get_var($wpdb->prepare("SELECT behavior_points FROM {$wpdb->prefix}sm_members WHERE id = %d", $member_id));

            // Thresholds for Member Case File
            if ($total_points >= 20 || ($degree >= 3 && $recurrence >= 1)) {
                $wpdb->update("{$wpdb->prefix}sm_members", array('case_file_active' => 1), array('id' => $member_id));
                SM_Logger::log('فتح ملف حالة عضو', "معرف العضو: $member_id بسبب وصول النقاط إلى $total_points");
            }
        }
    ...
    public static function get_record_by_id($id) {
        ...
        return $wpdb->get_row($wpdb->prepare(
            "SELECT r.*, s.name as member_name, s.class_name, s.section, s.member_code FROM {$wpdb->prefix}sm_records r JOIN {$wpdb->prefix}sm_members s ON r.member_id = s.id WHERE r.id = %d",
            $id
        ));
    }
    ...
    public static function get_records($filters = array()) {
        global $wpdb;
        $query = "SELECT r.*, s.name as member_name, s.class_name, s.section FROM {$wpdb->prefix}sm_records r JOIN {$wpdb->prefix}sm_members s ON r.member_id = s.id WHERE 1=1";
        
        if (!empty($filters['member_id'])) {
            $query .= $wpdb->prepare(" AND r.member_id = %d", $filters['member_id']);
        }
        ...
        if (!empty($filters['search'])) {
            ...
            $query .= $wpdb->prepare(" AND ($name_sql LIKE %s OR s.member_code LIKE %s)", $search_like, $search_like);
        }
    ...
    public static function delete_member($id) {
        ...
        if ($member) {
            SM_Logger::log('حذف عضو', 'ROLLBACK_DATA:' . json_encode(array('table' => 'members', 'data' => $member)));
            ...
            $wpdb->delete("{$wpdb->prefix}sm_assignments", array('member_id' => $id));
    ...
    public static function get_backup_data() {
        global $wpdb;
        $data = array(
            'members' => $wpdb->get_results("SELECT * FROM {$wpdb->prefix}sm_members", ARRAY_A),
            'records' => $wpdb->get_results("SELECT r.*, s.member_code FROM {$wpdb->prefix}sm_records r JOIN {$wpdb->prefix}sm_members s ON r.member_id = s.id", ARRAY_A),
            'attendance' => $wpdb->get_results("SELECT a.*, s.member_code FROM {$wpdb->prefix}sm_attendance a JOIN {$wpdb->prefix}sm_members s ON a.member_id = s.id", ARRAY_A),
            'confiscated_items' => $wpdb->get_results("SELECT c.*, s.member_code FROM {$wpdb->prefix}sm_confiscated_items c JOIN {$wpdb->prefix}sm_members s ON c.member_id = s.id", ARRAY_A)
        );
    ...
    public static function restore_backup($json) {
        ...
        // Cache for member code -> local ID
        $member_map = array();

        // 1. Process Members First
        if (isset($data['members'])) {
            foreach ($data['members'] as $member) {
                $exists = $wpdb->get_var($wpdb->prepare("SELECT id FROM {$wpdb->prefix}sm_members WHERE member_code = %s", $member['member_code']));
                if ($exists) {
                    unset($member['id']);
                    $wpdb->update("{$wpdb->prefix}sm_members", $member, array('id' => $exists));
                    $member_map[$member['member_code']] = $exists;
                } else {
                    unset($member['id']);
                    $wpdb->insert("{$wpdb->prefix}sm_members", $member);
                    $member_map[$member['member_code']] = $wpdb->insert_id;
                }
            }
        }

        // Helper to get member ID by code (local)
        $get_sid = function($code) use (&$member_map, $wpdb) {
            if (isset($member_map[$code])) return $member_map[$code];
            $id = $wpdb->get_var($wpdb->prepare("SELECT id FROM {$wpdb->prefix}sm_members WHERE member_code = %s", $code));
            if ($id) $member_map[$code] = $id;
            return $id;
        };
        ...
        if (isset($data['records'])) {
            foreach ($data['records'] as $record) {
                $local_sid = $get_sid($record['member_code'] ?? '');
                if (!$local_sid) continue;

                $old_id = $record['id'];
                unset($record['id'], $record['member_code']);
                $record['member_id'] = $local_sid;

                // Check if this specific record exists (by time and member and type)
                $exists = $wpdb->get_var($wpdb->prepare(
                    "SELECT id FROM {$wpdb->prefix}sm_records WHERE member_id = %d AND created_at = %s AND type = %s",
        ...
        if (isset($data['attendance'])) {
            foreach ($data['attendance'] as $att) {
                $local_sid = $get_sid($att['member_code'] ?? '');
                if (!$local_sid) continue;

                unset($att['id'], $att['member_code']);
                $att['member_id'] = $local_sid;

                $exists = $wpdb->get_var($wpdb->prepare("SELECT id FROM {$wpdb->prefix}sm_attendance WHERE member_id = %d AND date = %s", $local_sid, $att['date']));
                if ($exists) {
        ...
        if (isset($data['confiscated_items'])) {
            foreach ($data['confiscated_items'] as $item) {
                $local_sid = $get_sid($item['member_code'] ?? '');
                if (!$local_sid) continue;

                unset($item['id'], $item['member_code']);
                $item['member_id'] = $local_sid;

                $exists = $wpdb->get_var($wpdb->prepare(
                    "SELECT id FROM {$wpdb->prefix}sm_confiscated_items WHERE member_id = %d AND created_at = %s AND item_name = %s",
    ...
    public static function get_statistics($filters = array()) {
        ...
        $stats['by_class'] = $wpdb->get_results("SELECT s.class_name, COUNT(r.id) as count FROM {$wpdb->prefix}sm_records r JOIN {$wpdb->prefix}sm_members s ON r.member_id = s.id $where GROUP BY s.class_name");
        
        if (!empty($filters['teacher_id'])) {
            $stats['total_members'] = $wpdb->get_var($wpdb->prepare("SELECT COUNT(DISTINCT member_id) FROM {$wpdb->prefix}sm_records WHERE teacher_id = %d", $filters['teacher_id']));
        } else {
        ...
        $stats['top_members'] = $wpdb->get_results("
    ...
    public static function update_member_photo($id, $url) {
        global $wpdb;
        return $wpdb->update("{$wpdb->prefix}sm_members", array('photo_url' => $url), array('id' => $id));
    }
    ...
    public static function get_member_stats($member_id) {
        ...
        $stats['points'] = $wpdb->get_var($wpdb->prepare("SELECT behavior_points FROM {$wpdb->prefix}sm_members WHERE id = %d", $member_id));
        $stats['case_file'] = $wpdb->get_var($wpdb->prepare("SELECT case_file_active FROM {$wpdb->prefix}sm_members WHERE id = %d", $member_id));
        ...
        $stats['by_severity'] = $wpdb->get_results($wpdb->prepare("SELECT severity, COUNT(*) as count FROM {$wpdb->prefix}sm_records WHERE member_id = %d GROUP BY severity", $member_id));
        $stats['by_degree'] = $wpdb->get_results($wpdb->prepare("SELECT degree, COUNT(*) as count FROM {$wpdb->prefix}sm_records WHERE member_id = %d GROUP BY degree", $member_id));
        
        // Intelligence: Last action and frequent type
        $stats['last_action'] = $wpdb->get_var($wpdb->prepare("SELECT action_taken FROM {$wpdb->prefix}sm_records WHERE member_id = %d AND action_taken != '' ORDER BY created_at DESC LIMIT 1", $member_id));
    ...
    public static function get_confiscated_items($filters = array()) {
        global $wpdb;
        $query = "SELECT c.*, s.name as member_name, s.class_name, s.section FROM {$wpdb->prefix}sm_confiscated_items c
                  JOIN {$wpdb->prefix}sm_members s ON c.member_id = s.id WHERE 1=1";
        if (!empty($filters['member_id'])) {
            $query .= $wpdb->prepare(" AND c.member_id = %d", $filters['member_id']);
        }
        if (!empty($filters['status'])) {
    ...
    public static function add_confiscated_item($data) {
        global $wpdb;
        SM_Logger::log('تسجيل مادة مصادرة', "العضو: {$data['member_id']}، المادة: {$data['item_name']}");
        return $wpdb->insert(
        ...
        foreach ($db_structure as $grade_num => $sections) {
            ...
            foreach ($sections as $section) {
                ...
                $member_count = $wpdb->get_var($wpdb->prepare(
                    "SELECT COUNT(*) FROM {$wpdb->prefix}sm_members WHERE class_name = %s AND section = %s",
                    $class_name, $section
                ));
                ...
                $attendance_stats = $wpdb->get_results($wpdb->prepare(
                    "SELECT a.status, COUNT(*) as count
                     ...
                     JOIN {$wpdb->prefix}sm_members s ON a.member_id = s.id
                ...
                $summary[] = array(
                    ...
                    'stats' => $stats,
                    'is_complete' => ($stats['total_marked'] >= $member_count),
    ...
    public static function get_members_attendance($class_name, $section, $date) {
        global $wpdb;
        $query = $wpdb->prepare(
            "SELECT s.id, s.name, s.member_code, s.photo_url, a.status, a.id as attendance_id
    ...
    public static function save_attendance($member_id, $status, $date, $teacher_id) {
        global $wpdb;
        $exists = $wpdb->get_var($wpdb->prepare(
            "SELECT id FROM {$wpdb->prefix}sm_attendance WHERE member_id = %d AND date = %s",
            $member_id, $date
        ));
